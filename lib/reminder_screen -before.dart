...
// ── Key constants are now getters computed from email (see _ReminderScreenState) ──
...
class _ReminderScreenState extends State<ReminderScreen> {
  ...
  // ── Per-user email (loaded once in _initAndLoad) ─────────────────────────
  String _userEmail = 'guest';

  // ── Per-user SharedPreferences keys ─────────────────────────────────────
  String get _healthRemindersKey   => '${_userEmail}_health_record_reminders';
  String get _medicineRemindersKey => '${_userEmail}_medicine_reminders';
  ...
  Future<void> _initAndLoad() async {
    // ── Load the currently logged-in user's email first ──────────────────
    final prefs = await SharedPreferences.getInstance();
    final email = prefs.getString('loggedInUserEmail') ?? 'guest';
    _userEmail = email;
  ...
  Future<void> _loadReminders() async {
    ...
    // Uses per-user keys via the getters above
      ...
      if (r.enabled) {
        if (r.repeatType == 'weekly' && r.selectedDay != null) {
          await NotificationService.instance.scheduleWeekly(
            r.id, r.hour, r.minute, r.selectedDay!,
  ...
  Future<void> _addHealthReminder() async {
    ...
    if (reminder.repeatType == 'weekly' && reminder.selectedDay != null) {
      await NotificationService.instance.scheduleWeekly(
        reminder.id, reminder.hour, reminder.minute, reminder.selectedDay!,
        'Time to check your health',
        'Measure your heart rate, HRV or blood pressure',
        isMedicine: false,
      );
      ...
      await NotificationService.instance.scheduleDaily(
        reminder.id, reminder.hour, reminder.minute,
        'Time to check your health',
        'Measure your heart rate, HRV or blood pressure',
        isMedicine: false,
      );
  ...
  Future<void> _editHealthReminder(Reminder r) async {
    ...
    if (updated.enabled) {
      ...
      if (updated.repeatType == 'weekly' && updated.selectedDay != null) {
        await NotificationService.instance.scheduleWeekly(
          updated.id, updated.hour, updated.minute, updated.selectedDay!,
          'Time to check your health',
          'Measure your heart rate, HRV or blood pressure',
          isMedicine: false,
        );
        ...
        await NotificationService.instance.scheduleDaily(
          updated.id, updated.hour, updated.minute,
          'Time to check your health',
          'Measure your heart rate, HRV or blood pressure',
          isMedicine: false,
        );
          ...
          Expanded(
            child: Text(
              ...
              style: TextStyle(
                ...
                color:
                    reminder.enabled ? const Color(0xFF2D3748) : Colors.grey,
              ),
            ),
          ),
          // Edit button
