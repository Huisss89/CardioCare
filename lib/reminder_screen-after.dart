...
// Each account gets its own keys so reminders never bleed across accounts.
String _healthKey(String uid) => 'health_record_reminders_$uid';
String _medicineKey(String uid) => 'medicine_reminders_$uid';
...
class _ReminderScreenState extends State<ReminderScreen> {
  ...
  // ── Firebase UID getter ───────────────────────────────────────────────────
  // All SharedPreferences keys are scoped to this UID, so each Firebase
  // account has completely separate reminder data on this device.
  ...
  Future<void> _loadReminders() async {
    ...
    // Load reminders belonging to the currently signed-in account only
    final healthJson = prefs.getStringList(_healthKey(_uid)) ?? [];
    final medicineJson = prefs.getStringList(_medicineKey(_uid)) ?? [];
  ...
  Future<void> _saveHealthReminders() async {
    final prefs = await SharedPreferences.getInstance();
    // Persist under the current user's UID-scoped key
    await prefs.setStringList(_healthKey(_uid),
  ...
  Future<void> _saveMedicineReminders() async {
    final prefs = await SharedPreferences.getInstance();
    // Persist under the current user's UID-scoped key
    await prefs.setStringList(_medicineKey(_uid),
