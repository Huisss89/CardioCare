...
class _DashboardScreenState extends State<DashboardScreen> {
  ...
  // ── Health Score info dialog ─────────────────────────────────────────────
  void _showHealthScoreDialog() {
    final Color scoreColor = _healthScore >= 80
        ? const Color(0xFF48BB78)
        : _healthScore >= 60
            ? const Color(0xFFED8936)
            : const Color(0xFFF56565);

    final String scoreLabel = _healthScore >= 80
        ? 'Good'
        : _healthScore >= 60
            ? 'Fair'
            : 'Needs Attention';

    final String scoreDescription = _healthScore >= 80
        ? 'Your overall health metrics look great. Keep up the healthy habits!'
        : _healthScore >= 60
            ? 'Your health metrics are moderate. Small improvements can make a big difference.'
            : 'Some of your health metrics need attention. Consider consulting a healthcare professional.';

    // Derive individual metric summaries from available data
    final String hrSummary = _hrSeries.isNotEmpty
        ? 'Latest: ${_hrSeries.last.toInt()} BPM'
        : 'No data recorded yet';
    final String hrvSummary = _hrvSeries.isNotEmpty
        ? 'Latest: ${_hrvSeries.last.toStringAsFixed(1)} ms'
        : 'No data recorded yet';
    final String bpSummary =
        _lastBP != null ? 'Latest: $_lastBP mmHg' : 'No data recorded yet';
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          _scoreMetricRow(
                            ...
                            summary: hrSummary,
                            description:
                                'Measures how fast your heart beats per minute. Normal resting range is 60–100 BPM.',
                          ),
                          const SizedBox(height: 12),
                          ...
                          _scoreMetricRow(
                            ...
                            summary: hrvSummary,
                            description:
                                'HRV reflects how well your autonomic nervous system adapts. Higher values generally indicate better cardiovascular fitness.',
                          ),
                          const SizedBox(height: 12),
                          ...
                          _scoreMetricRow(
                            ...
                            summary: bpSummary,
                            description:
                                'Tracks systolic/diastolic pressure. Optimal is below 120/80 mmHg.',
                          ),
                          ...
                          Container(
                            ...
                            decoration: BoxDecoration(
                              ...
                              border: Border.all(
                                  color: _kPurple1.withOpacity(0.18)),
                            ),
                            child: Row(
                              crossAxisAlignment: CrossAxisAlignment.start,
                              children: [
                                Icon(Icons.lightbulb_outline_rounded,
                                    size: 18, color: _kPurple1),
                                const SizedBox(width: 10),
                                Expanded(
                                  child: Text(
                                    'Your score updates automatically each time you record a new measurement. More data = more accurate score.',
                                    style: TextStyle(
                                      fontSize: 12,
  ...
  /// A single metric row used inside the health score dialog
